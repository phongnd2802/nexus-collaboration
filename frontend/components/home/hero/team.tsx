...
export const TeamView = () => {
  const t = useTranslations("HomePage.hero.preview.team");
    ...
    >
      ...
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-sm font-medium text-foreground flex items-center">
          <Users className="h-3 w-3 mr-1 text-main" /> {t("title")}
        </h3>
        <span className="text-xs text-muted-foreground">
          {t("membersCount", { count: TEAM_MEMBERS.length })}
        </span>
      </div>
        ...
        {TEAM_MEMBERS.map((member, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`h-2 w-2 rounded-full ${statusColor} shadow-inner`}
              ></div>
            </div>
          );
        })}
