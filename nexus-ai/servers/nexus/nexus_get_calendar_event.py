...
async def nexus_get_calendar_event(
    workspace_id: str,
    event_id: str,
    ...
    Args:
        ...
        event_id: required; string; Calendar event ID.
    ...
    return await call_tool('nexus_get_calendar_event', arguments)
...
if __name__ == "__main__":
    ...
    async def test() -> None:
        print("Testing nexus_get_calendar_event...")
        try:
            result = await nexus_get_calendar_event(
                workspace_id=_get_env(['NEXUS_TEST_WORKSPACE_ID', 'NEXUS_WORKSPACE_ID']),
                event_id=_get_env(['NEXUS_TEST_EVENT_ID']),
            )
