...
async def nexus_create_calendar_event(
    workspace_id: str,
    data: dict[str, Any],
    ...
    Args:
        ...
        data: required; object; Request body matching the corresponding Nexus backend DTO.
    ...
    return await call_tool('nexus_create_calendar_event', arguments)
...
if __name__ == "__main__":
    ...
    async def test() -> None:
        print("Testing nexus_create_calendar_event...")
        ...
        try:
            result = await nexus_create_calendar_event(
                workspace_id=_get_env(['NEXUS_TEST_WORKSPACE_ID', 'NEXUS_WORKSPACE_ID']),
                data=_get_json(['NEXUS_TEST_DATA']),
            )
