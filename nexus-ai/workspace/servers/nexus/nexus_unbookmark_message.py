...
async def nexus_unbookmark_message(
    workspace_id: str,
    message_id: str,
    response_format: Optional[str] = None,
) -> str:
    """
    Remove bookmark from a message in a channel or conversation.
    ...
    return await call_tool('nexus_unbookmark_message', arguments)
...
if __name__ == "__main__":
    ...
    async def test() -> None:
        print("Testing nexus_unbookmark_message...")
        ...
        try:
            result = await nexus_unbookmark_message(
                workspace_id=_get_env(['NEXUS_TEST_WORKSPACE_ID', 'NEXUS_WORKSPACE_ID']),
                message_id=_get_env(['NEXUS_TEST_MESSAGE_ID']),
            )
