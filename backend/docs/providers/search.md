...
Nexus supports pluggable keyword search across workspace content
(workspaces, projects, tasks, channels, files, calendar events). Pick
a provider by setting `SEARCH_PROVIDER` in your `.env`.

Keyword search is separate from Nexus's existing Qdrant-backed
...
## Migration for existing Nexus callers
