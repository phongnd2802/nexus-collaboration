...
export async function getCalendarDataService(params: {
  userId: string;
  startDate?: string;
  endDate?: string;
}) {
...
export async function getDeadlinesService(params: { userId: string; daysAhead: number }) {
  const { userId, daysAhead } = params;
