...
function highlightReminderDuration(headline: string, urgencyColor: string): string {
  const match = headline.match(/(\d+\s+(?:ngày|giờ))/i);
  if (!match) return headline;

  const duration = match[1];
  return headline.replace(
    duration,
    `<strong style="color:${urgencyColor};font-weight:600">${duration}</strong>`,
  );
}
...
export function buildTaskReminderEmail(data: TaskReminderEmailData): { html: string; text: string } {
  ...
  const highlightedHeadline = highlightReminderDuration(data.reminderHeadline, urgencyColor);
  ...
  const html = [
    ...
    `<p style="margin:0 0 8px;color:${brandAccentColor};font-size:12px;line-height:16px;letter-spacing:0.25px;text-transform:uppercase">Task Reminder · ${urgencyLabel}</p>`,
    ...
    `<p style="margin:0 0 24px;color:#1F1E1D;font-size:15px;line-height:22.5px">${highlightedHeadline}.</p>`,
    `<div style="border:1px solid rgba(31,30,29,0.15);border-left:4px solid ${brandAccentColor};border-radius:12px;padding:20px 16px;background:#FFFFFF;margin-bottom:24px">`,
    ...
    `<tr><td style="padding:6px 0;color:#73726C;font-size:14px;line-height:19.6px;vertical-align:top">Mức ưu tiên</td><td style="padding:6px 0"><span style="display:inline-block;background:${brandAccentColor};color:#FFFFFF;padding:4px 12px;border-radius:8px;font-size:12px;line-height:16px;font-weight:600">${priorityLabel}</span></td></tr>`,
    `</table>`,
    `</div>`,
    `<a href="${data.taskUrl}" style="display:inline-block;background:${brandAccentColor};color:#FFFFFF;padding:12px 24px;text-decoration:none;border-radius:9.6px;font-size:15px;line-height:22.5px;font-weight:400">Xem chi tiết task</a>`,
    `</div>`,
