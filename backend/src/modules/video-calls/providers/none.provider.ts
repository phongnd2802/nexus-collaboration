...
export class NoneProvider implements VideoProvider {
  ...
  constructor() {
    this.logger.log(
      'Video conferencing is DISABLED (VIDEO_PROVIDER not set or set to "none"). To enable video, set VIDEO_PROVIDER to one of: jitsi, whereby, daily, livekit, agora.',
    );
  }
  ...
  private fail(op: string): never {
    throw new VideoProviderNotConfiguredError('none', [
      `VIDEO_PROVIDER (currently unset) - cannot ${op}`,
    ]);
  }

  async createRoom(_options: CreateRoomOptions): Promise<VideoRoom> {
    return this.fail('createRoom');
  }
  async getRoom(_roomId: string): Promise<VideoRoom | null> {
    return null;
  }
  ...
  async generateToken(_roomId: string, _options: TokenOptions): Promise<RoomToken> {
    return this.fail('generateToken');
  }
